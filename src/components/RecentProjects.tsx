...
const RecentProjects = () => {
  ...
  return (
    <div className="py-20 pt-32" id="projects">
        ...
        {projects.map(({ id, title, des, img, iconLists, link }) => (
          ...
          >
            <PinContainer title={link} href={link}>
              ...
              <p className="lg:text-base text-[#b5b7d3] pt-3 lg:font-normal font-light text-sm line-clamp-2">
                {des}
              </p>
              <div className="flex items-center justify-between mt-7 mb-3">
                <div className="flex items-center">
                  {iconLists.map((icon, index) => (
                    <div
                      key={icon}
                      className="border border-[#281f22] rounded-full bg-black lg:w-10 lg:h-10 sm:w-8 sm:h-8 flex justify-center items-center"
                      style={{ transform: `translateX(-${5 * index * 2}px)` }}
                    >
                      <Image src={icon} alt={icon} width={35} height={35} className="p-2" />
                    </div>
                  ))}
                </div>
